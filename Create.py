...
try:
    import subprocess
    HAS_LATEX = subprocess.run(['pdflatex', '--version'], capture_output=True).returncode == 0
except:
    HAS_LATEX = False
...
class AustrianData:
    ...
    firmen = [
        'voestalpine', 'Siemens Österreich', 'Magna Steyr', 'AVL List',
        'Andritz AG', 'Palfinger', 'Rosenbauer', 'KTM', 'Doppelmayr'
    ]
...
class MathSolver:
    ...
    def add_fractions(f1: Fraction, f2: Fraction) -> Tuple[Fraction, int]:
        ...
        result = f1 + f2
        common_denominator = f1.denominator * f2.denominator // math.gcd(f1.denominator, f2.denominator)
...
class QualityControl:
    ...
    def check_similarity(self, numbers: List[int]) -> bool:
        ...
        for prev_numbers in self.used_numbers[-5:]:  # Check last 5
            if self._calculate_similarity(numbers, prev_numbers) > self.similarity_threshold:
                return False
        return True
    ...
    def _calculate_similarity(self, list1: List[int], list2: List[int]) -> float:
        ...
        matches = sum(1 for a, b in zip(sorted(list1), sorted(list2)) if abs(a - b) < max(a, b) * 0.1)
    ...
    def validate_range(self, value: float, min_val: float, max_val: float, name: str) -> bool:
        """Validiert ob Wert in realistischem Bereich liegt"""
        if not min_val <= value <= max_val:
            print(f"Warnung: {name} = {value} außerhalb des Bereichs [{min_val}, {max_val}]")
            return False
        return True
...
class UnitConverter:
    ...
    conversions = {
        ...
        # Fläche (Basis: m²)
        'mm²': 0.000001, 'cm²': 0.0001, 'dm²': 0.01, 'm²': 1, 'km²': 1000000,
        'ha': 10000, 'a': 100,
        
        # Volumen (Basis: m³)
        'mm³': 0.000000001, 'cm³': 0.000001, 'dm³': 0.001, 'm³': 1,
        'ml': 0.000001, 'cl': 0.00001, 'dl': 0.0001, 'l': 0.001, 'hl': 0.1,
...
class AufgabenGenerator:
    ...
    def _template_addition(self) -> Tuple[str, str, str]:
        """Addition Template"""
        a = random.randint(50, 500) if self.schwierigkeit == Schwierigkeit.EINFACH else random.randint(100, 999)
        b = random.randint(20, 200) if self.schwierigkeit == Schwierigkeit.EINFACH else random.randint(50, 500)
        c = random.randint(10, 100) if self.schwierigkeit == Schwierigkeit.EINFACH else random.randint(20, 300)
    ...
    def _template_verschachtelt1(self) -> Tuple[str, str, str]:
        ...
        loesung = str(result)
        erklaerung = (f"Schritt 1: {d} - {e} = {inner}\n"
    ...
    def _template_verschachtelt2(self) -> Tuple[str, str, str]:
        ...
        loesung = str(result)
        erklaerung = (f"Schritt 1: {b} · {c} = {mult}\n"
    ...
    def _template_negativ(self) -> Tuple[str, str, str]:
        ...
        loesung = str(result)
        erklaerung = (f"Schritt 1: (-{b}) · {c} = {neg_mult}\n"
        ...
        if niveau == 1:
            ...
            if operation == '·':
                ...
                if result.numerator != n1*n2 or result.denominator != d1*d2:
                    erklaerung += f" = {result.numerator}/{result.denominator} (gekürzt)"
            else:
                result = f1 / f2
                aufgabe = f"({n1}/{d1}) : ({n2}/{d2})"
    ...
    def generate_gleichung(self, schwer: bool = False) -> Tuple[str, str, str]:
        ...
        if not schwer:
            ...
            a1, b1, c1 = random.randint(2, 5), random.randint(1, 4), random.randint(2, 10)
            ...
            if coeff_x != 0:
                x = -const / coeff_x
                loesung = f"x = {x:.2f}"
                erklaerung = f"Ausmultiplizieren und zusammenfassen: {coeff_x}x + {const} = 0"
            else:
            ...
            c1, d1, e1 = random.randint(2, 8), random.randint(5, 20), random.randint(2, 8)
            
            aufgabe = f"{a1}({b1}x - {dec}) + {frac_num}/{frac_den}·(x + {c1}) = {d1}/{e1}"
    ...
    def _template_gehalt(self) -> Tuple[str, str, str]:
        ...
        aufgabe = (f"Ein {beruf.replace('_', '-').title()} verdient {gehalt}€ bei {stunden_alt} Stunden/Woche. "
        ...
        erklaerung = f"Stundenlohn: {stundenlohn:.2f}€/h, Neues Gehalt: {neues_gehalt:.2f}€"
    ...
    def _template_material(self) -> Tuple[str, str, str]:
        ...
        aufgabe = (f"Ein Werkstück aus {material.replace('_', ' ').title()} hat die Maße "
    ...
    def _template_produktion(self) -> Tuple[str, str, str]:
        ...
        aufgabe = (f"{maschinen} CNC-Fräsmaschinen produzieren einen Auftrag in {zeit} Stunden. "
    ...
    def _template_pumpsystem(self) -> Tuple[str, str, str]:
        ...
        aufgabe = (f"Ein Tank fasst {tank}L. Pumpe A füllt ihn in {pumpe_a_zeit}min, "
    ...
    def _template_mischung(self) -> Tuple[str, str, str]:
        ...
        aufgabe = (f"Eine Metalllegierung wird aus zwei Sorten gemischt: "
    ...
    def _template_logistik(self) -> Tuple[str, str, str]:
        ...
        aufgabe = (f"Ein LKW (Nutzlast {lkw_kapazitaet}kg) soll {paletten} Paletten à {gewicht_palette}kg "
    ...
    def _zahl_zu_text(self, zahl: int) -> str:
        ...
        einer_namen = ["", "ein", "zwei", "drei", "vier", "fünf", "sechs", "sieben", "acht", "neun"]
        zehner_namen = ["", "zehn", "zwanzig", "dreißig", "vierzig", "fünfzig", 
    ...
    def generate_runden(self) -> Tuple[str, str, str]:
        ...
        for i, (zahl, stelle, gerundet) in enumerate(zip(zahlen, stellen, gerundete), 1):
    ...
    def generate_geometrie(self, zeichnen: bool = True) -> Tuple[str, str, str]:
        ...
        if zeichnen:
            ...
            aufgabe = (f"Ein L-förmiges Werkstück besteht aus zwei Rechtecken:\n"
            ...
            aufgabe = (f"Ein {material.title()}träger: {laenge}cm × {breite}cm × {hoehe}cm\n"
                      f"Dichte: {dichte}kg/dm³, Preis: {preis}€/kg\n"
                      f"a) Gewicht? b) Materialkosten? c) Mit {verschnitt}% Verschnitt?")
            
            volumen = self.geometry.cuboid_volume(laenge/10, breite/10, hoehe/10)  # in dm³
            gewicht = volumen * dichte
            kosten = gewicht * preis
            kosten_verschnitt = kosten * (1 + verschnitt/100)
            
            loesung = f"a) {gewicht:.2f}kg, b) {kosten:.2f}€, c) {kosten_verschnitt:.2f}€"
...
"""
    ...
    def _add_zahlenraum(self):
        ...
        self.test_content += "Tragen Sie folgende Werte ein: 0,5; -2,8; 6; 1/2; -3/4\n\n"
        self.test_content += "```\n"
        ...
        for niveau, punkte, titel in [(1, 2, "Leicht"), (2, 2, "Mittel"), (3, 3, "Schwer")]:
    ...
    def _add_raumvorstellung(self):
        ...
        self.test_content += f"**c) Geometrische Berechnung und Zeichnung (5 Punkte)**\n{aufgabe}\n\n"
        self.solutions += f"**c)** {loesung}\n   {erklaerung}\n\n"
...
class OutputManager:
    ...
    @staticmethod
    def save_word(test_content: str, solutions: str, test_name: str = "ueberstiegstest"):
        """Erstellt professionelle Word-Dokumente"""
        if not HAS_DOCX:
            print("❌ Word-Export nicht verfügbar. Installieren Sie: pip install python-docx")
            return
        ...
        doc.add_heading('Technische Basisausbildung', 1).alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    @staticmethod
    def save_latex(test_content: str, solutions: str, test_name: str = "ueberstiegstest"):
        ...
        if HAS_LATEX:
            try:
                ...
                subprocess.run(['pdflatex', f'{test_name}_loesungen.tex'], capture_output=True)
